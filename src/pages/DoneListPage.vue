...
const { doneList } = storeToRefs(useListsStore())


const currentBookList = ref<CopyBook[]>(doneList.value)
const searchValue = ref<string>('')
const emptySearchResponse = ref<boolean>(false)

const searchHandler = debounce(() => {
    currentBookList.value = searchInList(searchValue.value, doneList.value)
...
<template>
    <AppLayout :navigtionButtons="false">
        ...
        <template #search-input>
            <n-input v-model:value="searchValue" :input-props="{ id: 'doneSearch' }" type="text" size="large" round
